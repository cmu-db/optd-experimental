...
async fn test_group_merge_ladder() {
    ...
    let (scan_id, _) = memo.add_group(scan_base, &[]).await.unwrap().ok().unwrap();

    let filter0 = filter(scan_id, "true".to_string());
    let (filter_id_0, _) = memo
    ...
    let m0 = memo.merge_groups(filter_id_3, filter_id_2).await.unwrap();
    let m1 = memo.merge_groups(filter_id_2, filter_id_1).await.unwrap();
    let m2 = memo.merge_groups(filter_id_1, filter_id_0).await.unwrap();
    let root = memo.merge_groups(filter_id_0, scan_id).await.unwrap();
    groups.extend_from_slice(&[m0, m1, m2, root]);

    for group_id in groups {
        assert_eq!(root, memo.get_root_group(group_id).await.unwrap());
    }
...
async fn test_group_merge() {
    ...
    let (filter_id_3, _) = memo
        ...
        .ok()
        .unwrap();
    let (filter_id_4, _) = memo
        .add_group(filter4, &[scan_id_1])
        .await
        .unwrap()
        .ok()
        .unwrap();
    let (filter_id_5, _) = memo
        .add_group(filter5, &[scan_id_1])
        .await
        .unwrap()
        .ok()
        .unwrap();
    let (filter_id_6, _) = memo
        .add_group(filter6, &[scan_id_1])
        .await
        .unwrap()
        .ok()
        .unwrap();
    let (filter_id_7, _) = memo
    ...
    let quarter_0 = memo.merge_groups(filters[0], filters[1]).await.unwrap();
    let quarter_1 = memo.merge_groups(filters[2], filters[3]).await.unwrap();
    let quarter_2 = memo.merge_groups(filters[4], filters[5]).await.unwrap();
    let quarter_3 = memo.merge_groups(filters[6], filters[7]).await.unwrap();
    let semi_0 = memo.merge_groups(quarter_0, quarter_1).await.unwrap();
    let semi_1 = memo.merge_groups(quarter_2, quarter_3).await.unwrap();
    let final_id = memo.merge_groups(semi_0, semi_1).await.unwrap();

    // Check that the group set is properly representative.
    {
        let set = memo.get_group_set(final_id).await.unwrap();
        assert_eq!(set.len(), 8);
        for id in set {
    ...
    let (join_group_id, join_expr_id) = memo
    ...
    for filter_id in filters {
        ...
        let (join_group_id_test, join_expr_id_test) = memo
...
async fn test_cascading_merge() {
    ...
    let scan1 = scan("t1".to_string());
    let (g1, _) = memo.add_group(scan1, &[]).await.unwrap().ok().unwrap();
    let scan2 = scan("t2".to_string());
    let (g2, _) = memo.add_group(scan2, &[]).await.unwrap().ok().unwrap();

    let filter1 = filter(g1, "x > 1000".to_string());
    let (g3, _) = memo.add_group(filter1, &[g1]).await.unwrap().ok().unwrap();

    // Create two groups that will need to be merged.
    let filter2a = filter(g2, "a < 42".to_string());
    let (g4, _) = memo.add_group(filter2a, &[g2]).await.unwrap().ok().unwrap();
    let filter2b = filter(g4, "a < 42 AND 1 = 1".to_string());
    let (g5, _) = memo.add_group(filter2b, &[g4]).await.unwrap().ok().unwrap();

    // Create groups that are dependent on the to-be-merged groups.
    let join1 = join(g3, g4, "t1.x = t2.a".to_string());
    let (g6, _) = memo
        .add_group(join1, &[g3, g4])
        .await
        .unwrap()
        .ok()
        .unwrap();
    ...
    let (g8, _) = memo.add_group(sort1, &[g6]).await.unwrap().ok().unwrap();

    let sort2 = filter(g7, "ORDER BY a".to_string());
    let (g9, _) = memo.add_group(sort2, &[g7]).await.unwrap().ok().unwrap();

    // Now that everything is set up, we can merge groups 4 and 5 to begin the cascading process.
    let filter_root = memo.merge_groups(g4, g5).await.unwrap();
    assert_eq!(memo.get_root_group(g4).await.unwrap(), filter_root);
    assert_eq!(memo.get_root_group(g5).await.unwrap(), filter_root);
    ...
    let (existing_g6, existing_id) = memo
    ...
    // Since the memo table has told us these are duplicates, we can now merge groups 6 and 7.
    let join_root = memo.merge_groups(g6, g7).await.unwrap();
    assert_eq!(memo.get_root_group(g6).await.unwrap(), join_root);
    assert_eq!(memo.get_root_group(g7).await.unwrap(), join_root);
    ...
    // somewhere in the memo table before deciding to merge groups.
    let sort_root = memo.merge_groups(g8, g9).await.unwrap();
    assert_eq!(memo.get_root_group(g8).await.unwrap(), sort_root);
    assert_eq!(memo.get_root_group(g9).await.unwrap(), sort_root);
