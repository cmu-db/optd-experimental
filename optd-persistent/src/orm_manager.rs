...
impl StorageLayer for ORMManager {
    ...
    async fn get_stats_for_table(
        ...
        match epoch_id {
            Some(epoch_id) => Statistic::find()
                .filter(statistic::Column::TableId.eq(table_id))
                .filter(statistic::Column::StatisticType.eq(stat_type))
                .filter(statistic::Column::EpochId.eq(epoch_id))
                .one(&self.db_conn)
                .await
                .map(|stat| stat.map(|s| s.statistic_value)),

            None => Statistic::find()
                .filter(statistic::Column::TableId.eq(table_id))
                .filter(statistic::Column::StatisticType.eq(stat_type))
                .order_by_desc(statistic::Column::EpochId)
                .one(&self.db_conn)
                .await
                .map(|stat| stat.map(|s| s.statistic_value)),
        }
    ...
    async fn get_stats_for_attr(
        ...
        match epoch_id {
            Some(epoch_id) => Statistic::find()
                .filter(statistic::Column::NumberOfAttributes.eq(1))
                .filter(statistic::Column::StatisticType.eq(stat_type))
                .filter(statistic::Column::EpochId.eq(epoch_id))
                .inner_join(statistic_to_attribute_junction::Entity)
                .filter(statistic_to_attribute_junction::Column::AttributeId.eq(attr_id))
                .one(&self.db_conn)
                .await
                .map(|stat| stat.map(|s| s.statistic_value)),
    ...
    async fn get_stats_for_attrs(
        ...
        let attr_count = attr_ids.len() as i32;
        match epoch_id {
            Some(epoch_id) => Statistic::find()
                .filter(statistic::Column::NumberOfAttributes.eq(attr_count))
                .filter(statistic::Column::StatisticType.eq(stat_type))
                .filter(statistic::Column::EpochId.eq(epoch_id))
                .inner_join(statistic_to_attribute_junction::Entity)
                .filter(statistic_to_attribute_junction::Column::AttributeId.is_in(attr_ids))
                .group_by(statistic::Column::Id)
                .having(Expr::col(statistic::Column::Name).count().eq(attr_count))
                .one(&self.db_conn)
                .await
                .map(|stat| stat.map(|s| s.statistic_value)),
